...
The goal of coreml is to convert models trained/fitted in R to [coreml](https://developer.apple.com/documentation/coreml) so that you can use them in Apple iOS applications.

This is work in progress and just a proof of concept. Do not use it for production.
...
* `convert_keras` convert a keras model to coreml using [coremltools](https://github.com/apple/coremltools). This does not work at the moment.

## Example

Here we convert a logistic regression model (GLM with `family=binomial`) to a coreml model.
Save it to disk, read it in again and make predictions.
